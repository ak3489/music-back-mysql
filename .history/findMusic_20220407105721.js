...
dosinger((index)=>{
    ...
    conn.query(sql, async(err, result)=>{
        ...
        if(result.length==0){
            // 新增歌手
            const addsingersql = "insert into singer values(null,?,null,null,null,null)";//写追加数据的sql变量可用"?"占位，追加的字段数量要与刚才查询的字段数量一致，否则会报错 now()
            conn.query(addsingersql,[singerValues[index]],async(error,result)=>{//数组中放我们传入的变量会自动替换"?"
