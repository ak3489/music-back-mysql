...
//     const sql = `select * from album `;//查询album表中的专辑
    ...
    for (let index = 0; index < albumValues.length; index++) {
        ...
        conn.query(sql, async(err, result)=>{
            ...
            if(result.length==0){
                ...
                conn.query(addsql,[des, album_name, album_pic],async(error,result)=>{//数组中放我们传入的变量会自动替换"?"
                    if(error) throw error;
                    await res.status(200).send("添加成功")
                    conn.release();
                })
            }else{
                await res.status(200).send('专辑已经存在')
            }
        })
    }
    conn.release();//数据查询成功后归还连接
