...
pool.getConnection((err, conn)=> {
    ...
    function dosinger(i,album_id){
        //处理歌手
        let singerName = fileNames[i].artist||'undefined'
        let selectsinger = `select * from singer WHERE name = '${singerName}' `;//查询singer表中的歌手
        conn.query(selectsinger, async(err, result)=>{
            ...
            if(result.length==0){
                ...
                conn.query(addsql,[fileNames[i].artist||'undefined'],async(error,result)=>{//数组中放我们传入的变量会自动替换"?"
                    ...
                    await dosingeralbum(album_id,singer_id);
    ...
    function dosong(singer_id,i,album_id){
        let selectsong = `select * from song WHERE title = '${fileNames[i].title}' AND album_id = '${album_id}' `;//查询singer表中的歌手
        conn.query(selectsong, async(err, result)=>{
            ...
            if(result.length==0){
                ...
                conn.query(addsql,[fileNames[i].fPath,album_id||'undefined',fileNames[i].title],async(error,result)=>{//数组中放我们传入的变量会自动替换"?"
                    if(error) throw error;
                    console.log(`添加歌曲-${fileNames[i].title}-成功`)
                    let song_id = result.insertId;
                    console.table({'song_id':song_id,'singer_id':singer_id})
                    await dosongsinger(song_id,singer_id);
                })
            }else{
    ...
    for(let i=0; i<fileNames.length; i++){
        album_id = null;
        singer_id = null;
        let albumName = fileNames[i].album||'undefined';
        let albumsql = `select * from album WHERE album_name = '${albumName}' `;//查询album表中的专辑
        conn.query(albumsql, async(err, result)=>{
            ...
            if(result.length==0){
                ...
                conn.query(addalbumsql,[fileNames[i].album||'undefined'],async(error,result)=>{//数组中放我们传入的变量会自动替换"?"
                    if(error) throw error;
                    console.log(`添加专辑-${fileNames[i].album}-成功`)
                    album_id = result.insertId;
                    console.log('album_id',album_id);
                   await dosinger(i,album_id);
                })
            }else{
                let album_id = result[0].id;
                console.log(`专辑${fileNames[i].album}已经存在`)
                console.log('album_id222222',album_id);
                //处理歌手
                await dosinger(i,album_id);
