...
pool.getConnection((err, conn)=> {
    ...
    //处理歌曲
    function dosong(singer_id,i){
        let selectsong = `select * from song WHERE title = '${fileNames[i].title}' AND album_name = '${fileNames[i].album}' `;//查询singer表中的歌手
        conn.query(selectsong, async(err, result)=>{
            if (err) throw err;
            if(result.length==0){
                // 新增歌手
                const addsql = "insert into singer values(null,?,?,?,null,null)";//写追加数据的sql变量可用"?"占位，追加的字段数量要与刚才查询的字段数量一致，否则会报错 now()
                conn.query(addsql,[fileNames[i].fPath,fileNames[i].album,fileNames[i].title],async(error,result)=>{//数组中放我们传入的变量会自动替换"?"
                    if(error) throw error;
                    console.log(`添加歌曲-${fileNames[i].title}-成功`)
                    let song_id = result.insertId;
                    console.table({'song_id':song_id,'singer_id':singer_id})
                    await dosingeralbum(song_id,singer_id);
                })
            }else{
