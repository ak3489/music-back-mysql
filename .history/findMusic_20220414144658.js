    ...
    function finder(path) {
        ...
        files.forEach(async(val,index) => {
            ...
            //   console.log('fPath',fPath); 
                ...
                // mm.parseFile(fPath,{ duration: true}).then((e)=>{
                //     console.log('aaaa',e);
                //     // console.log('aaaa',e.format.duration);
                // });
                console.log('path.extname(fPath)',spath.extname(fPath));
                if (spath.extname(fPath).toLowerCase() !== '.mp3') continue;
                let mdata = await mm.parseFile(fPath,{ duration: true});
                // console.log('mdata===',mdata.format.duration);
                // console.log('tags.raw',tags.raw); 
                ...
                // console.log('tags',Object.getOwnPropertyNames(tags.raw).length)
                if(Object.getOwnPropertyNames(tags.raw).length>0){
                    musicName.push(data); 
                }            
            } 
        });
    ...
    function dosong(singer_id,i,album_id){
        ...
        conn.query(selectsong, async(err, result)=>{
            ...
            if(result.length==0){
                ...
                conn.query(addsql,[fileNames[i].fPath,album_id||'undefined',fileNames[i].title||'undefined',fileNames[i].duration],async(error,result)=>{//数组中放我们传入的变量会自动替换"?"
                    if(error) throw error;
