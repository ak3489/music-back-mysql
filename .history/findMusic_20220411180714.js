...
pool.getConnection(async (err, conn)=> {
    ...
    for(let i=0; i<fileNames.length; i++){
        ...
        const fhfhhf = await conn.promise().query(albumsql);
        console.log('fhfhhf==',fhfhhf);
        if(fhfhhf[0].length==0){
            const addalbumsql = "insert into album values(null,null,?,null)";//写追加数据的sql变量可用"?"占位，追加的字段数量要与刚才查询的字段数量一致，否则会报错 now()
            const tter = await conn.promise().query(addalbumsql,[fileNames[i].album||'undefined']);
            // console.log('tter',tter);
        }
        ...
        //         // 新增专辑
        //         const addalbumsql = "insert into album values(null,null,?,null)";//写追加数据的sql变量可用"?"占位，追加的字段数量要与刚才查询的字段数量一致，否则会报错 now()
        //         conn.promise().query(addalbumsql,[fileNames[i].album||'undefined'],async(error,result)=>{//数组中放我们传入的变量会自动替换"?"
        //             if(error) throw error;
