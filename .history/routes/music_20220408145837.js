...
router.get('/albumDetails', function(req, res, next) {
    ...
    pool.getConnection((err, conn)=> {
        ...
        let sqla = `select a.album_name from album a where a.id=${req.query.id}`;
        let sqlb = `select a.album_name,c.name
        from album a,singer_album b,singer c
        where a.id = b.album_id
        and b.singer_id = c.id
        and a.id=${req.query.id}`;
        let sqlc = `select a.album_name,d.fpath,d.title
        from album a,song d
        where a.id = d.album_id
        and a.id=${req.query.id}`;
        conn.query(sqla,sqlb,sqlc, async(err, result)=>{
            if (err) throw err;
            console.log('result',result);
            await res.send({
                code: 200,
