...
router.get('/albumList', function(req, res, next) {
    pool.getConnection((err, conn)=> {
        const sql = `select * from album limit ${req.query.limit} ${req.query.pageNo},${req.query.pageSize}`;//查询jd_user表中的用户
        conn.query(sql, async(err, result)=>{
            if (err) throw err;
            await res.send({
                code: 200,
...
router.get('/albumDetails', function(req, res, next) {
    ...
    pool.getConnection((err, conn)=> {
        ...
        let sqlc = `select a.album_name,d.fpath,d.title,d.song_pic,d.id,d.track_number,d.duration
        from album a,song d
