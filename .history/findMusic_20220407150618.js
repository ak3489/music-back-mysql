...
pool.getConnection((err, conn)=> {
    ...
    function dosingeralbum(album_id,singer_id){
        //处理歌手
        // let sql = `select * from singer_album WHERE album_id = '${album_id}' `;//查询singer表中的歌手
        let sql = 'SELECT * FROM `singer_album` WHERE `album_id` = album_id AND `singer_id` = singer_id';
        conn.query(selectsinger, async(err, result)=>{
            if (err) throw err;
            if(result.length==0){
                // singer_album
                const addsql = "insert into singer values(null,?,?)";//写追加数据的sql变量可用"?"占位，追加的字段数量要与刚才查询的字段数量一致，否则会报错 now()
